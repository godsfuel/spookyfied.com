...
# Define obscure, anonymous system-level paths (zero mention of spooky on disk)
...
if (Test-Path $StealthFilePath) {
    Write-Host "[+] Terminal Node already initialized and verified!" -ForegroundColor Green
...
# 2. Extract hardware signatures (CPU ID and Motherboard Serial via CIM)
$CpuId = (Get-CimInstance -ClassName Win32_Processor | Select-Object -ExpandProperty ProcessorId)
$MbSerial = (Get-CimInstance -ClassName Win32_BaseBoard | Select-Object -ExpandProperty SerialNumber)
